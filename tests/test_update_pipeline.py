...
def test_run_update_pipeline_orchestrates_full_refresh(monkeypatch):
    ...
    monkeypatch.setattr(update_module, "run_canonical_index", fake_canonical)
    ...
    assert calls == ["worldcup2026", "espn", "365scores", "canonical", "reports:finalizado", "status:canonical", "scores"]
...
def test_run_update_pipeline_can_skip_sources(monkeypatch):
    ...
    monkeypatch.setattr(update_module, "run_canonical_index", lambda: calls.append("canonical") or {})
    ...
    assert calls == ["canonical", "reports:todos", "status:canonical", "scores"]
